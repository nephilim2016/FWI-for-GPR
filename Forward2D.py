...
def Forward_2D(sigma,epsilon,mu,para):
    ...
    for index,value in enumerate(para.source_site):
        res=pool.apply_async(Forward2D,args=(para.xl,para.zl,para.Freq,para.k_max,sigma,epsilon,mu,para.dx,para.dz,para.dt,value,para.receiver_site,index))
        res_l.append(res)
    pool.close()
    pool.join()
    for res in res_l:
        result=res.get()
        np.save('./%sHz_forward_data_file/%sx_%sz_record.npy'%(para.Freq,result[0][0],result[0][1]),result[1])
        del result
    del res_l
    pool.terminate() 
...
def Forward2D(xl,zl,Freq,k_max,sigma,epsilon,mu,dx,dz,dt,value_source,value_receiver,index):
    ...
    source_site,Profile=time_loop(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,f,k_max,value_source,value_receiver)
    return source_site,Profile
...
if __name__=='__main__':
    ...
    dt=5e-11
    Freq=1e8
    epsilon,sigma=Create_Model.Abnormal_Model(xl,zl,10)
    ...
    source_site=[]
    receiver_site=[]
    for index in range(10,111,5):
        source_site.append((10,index))
    for index in range(10,111,5):
        source_site.append((110,index))
    for index in range(15,110,5):
        source_site.append((index,10))
    for index in range(15,110,5):
        source_site.append((index,110))
    
    for index in range(10,111,1):
        receiver_site.append((10,index))
    for index in range(10,111,1):
        receiver_site.append((110,index))
    for index in range(11,110,1):
        receiver_site.append((index,10))
    for index in range(11,110,1):
        receiver_site.append((index,110))
        
    ref_pos=np.array(receiver_site)
    source_site=[10,60]
    a,b=time_loop(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,f,k_max,source_site,ref_pos)
